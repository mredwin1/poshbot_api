...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for user in users:
            listed_items = ListedItem.objects.filter(
                posh_user__user=user, datetime_listed__isnull=False
            ...
            if options["start-date"]:
                ...
                listed_items.filter(
                    datetime_listed__gte=datetime.datetime(
            ...
            listed_items = listed_items.annotate(
                time_to_be_removed=time_to_be_removed_expression
            )

            average_duration = listed_items.aggregate(
                average_duration=Avg("time_to_be_removed")
            )["average_duration"]
        ...
        for user, average_duration in user_avg_durations:
            logger.info(
                f"The average time for {user} is: {self.format_duration(average_duration)}"
            )
