...
class Captcha:
    ...
    def send_captcha(self):
        ...
        if response["status"] == 1:
            ...
            self.logger.error(
                f'reCaptcha request failed! Error code: {response["request"]}'
            )
            return False
...
class BaseClient:
    def __init__(self, logger, proxy: Proxy = None):
        ...
        if proxy:
            ...
            self.web_driver_options.add_argument(
    ...
    def open(self):
        ...
        self.web_driver = webdriver.Chrome(
            service=service, options=self.web_driver_options
        )
        stealth(
            self.web_driver,
            languages=["en-US", "en"],
    ...
    def bot_check(self):
        ...
        if tests_failed:
            ...
            for failed_test in failed_tests:
                ...
                self.logger.info(
                    f"Failed Test: {failed_test_name.text}\n\nTest Data Results:\n{failed_test_data.text}"
                )
                failed_test_close_btn = self.locate(
                    By.XPATH,
                    "/html/body/section/div/div/div/div/div[3]/div[2]/header/button",
                )
...
class PoshMarkClient(BaseClient):
    ...
    def check_for_errors(self):
        ...
        for present_error_class in present_error_classes:
            if "form__error" in present_error_class:
                ...
                self.logger.error(
                    f"The following form errors were found: {','.join(error_texts)}"
                )
                ...
                if error.text == "Invalid Username or Password":
                    ...
                    self.web_driver.save_screenshot(
                        f"/log_images/{self.campaign.title}/captcha.png"
                    )
                    self.logger.warning("Captcha encountered")
                    captcha_iframe = self.locate(
                        By.TAG_NAME, "iframe", location_type="visibility"
                    )
                    captcha_src = captcha_iframe.get_attribute("src")
                    google_key = re.findall(r"(?<=k=)(.*?)(?=&)", captcha_src)[0]

                    captcha_solver = Captcha(
                        google_key, self.web_driver.current_url, self.logger
                    )
                    ...
                    while captcha_response is None and retries != 5:
                        self.logger.warning(
                            "Captcha not solved. Retrying captcha again..."
                        )
                        captcha_response = captcha_solver.solve_captcha()
                        retries += 1
                    ...
                    if retries == 5 and captcha_response is None:
                        self.logger.error(
                            f"2Captcha could not solve the captcha after {retries} attempts"
                        )
                    elif captcha_response == -1:
                        self.logger.error(
                            "Exiting after encountering an error with the captcha."
                        )
                    else:
                        word = "attempt" if retries == 1 else "attempts"
                        self.logger.info(
                            f"2Captcha successfully solved captcha after {retries} {word}"
                        )
                        # Set the captcha response
                        self.web_driver.execute_script(
                            f'grecaptcha.getResponse = () => "{captcha_response}"'
                        )
    ...
    def check_listing_timestamp(self, listed_item_id, listed_item_tile):
        ...
        try:
            self.logger.info(
                f"Checking the timestamp on following item: {listed_item_tile}"
            )
            ...
            timestamp_element = self.locate(
                By.XPATH,
                '//*[@id="content"]/div/div/div[3]/div[2]/div[1]/div[1]/header/div/div/div/div',
            )
            ...
            if elapsed_time > 60:
                ...
                self.logger.warning(
                    f"Sharing does not seem to be working "
            ...
            self.handle_error(
                "Error while checking listing timestamp", "listing_timestamp_error.png"
            )
    ...
    def check_inactive(self):
        ...
        try:
            self.logger.info(
                f"Checking is the following user is inactive: {self.posh_user.username}"
            )

            self.go_to_closet()
            listing_count_element = self.locate(
                By.XPATH,
                '//*[@id="content"]/div/div/div[1]/div/div/div[2]/div/div/nav/ul/li[1]/a',
            )
            ...
            if total_listings > 0 and not self.is_present(By.CLASS_NAME, "card--small"):
                self.logger.warning(
                    "This user does not seem to be active, setting inactive"
                )
                self.posh_user_inactive()
                return True
            else:
        ...
        try:
            ...
            for listed_item in listed_items:
                ...
                if title.text == listing_title:
                    listing_button = listed_item.find_element(
                        By.CLASS_NAME, "tile__covershot"
                    )
                    listing_button.click()

                    self.sleep(1)

                    edit_listing_button = self.locate(
                        By.XPATH, '//*[@id="content"]/div/div/div[3]/div[2]/div[1]/a'
                    )
            ...
            self.handle_error(
                "Error while deleting listing", "delete_listing_error.png"
            )
    ...
    def check_logged_in(self):
        ...
        try:
            ...
            if result:
                self.web_driver.save_screenshot(
                    f"/log_images/{self.campaign.title}/logged_in.png"
                )
                self.logger.info("User is logged in")
            else:
                self.web_driver.save_screenshot(
                    f"/log_images/{self.campaign.title}/not_logged_in.png"
                )
    ...
    def finish_registration(self):
        try:
            self.web_driver.save_screenshot(
                f"/log_images/{self.campaign.title}/registration_finished.png"
            )
            ...
            if self.posh_user.profile_picture:
                ...
                profile_picture_name = self.posh_user.profile_picture.name.split("/")[
                    -1
                ]
                self.bucket.download_file(
                    self.posh_user.profile_picture.name, profile_picture_name
                )

                profile_picture = self.locate(
                    By.XPATH, '//*[@id="content"]/div/div[2]/div[1]/label/input'
                )
            ...
            apply_button = self.locate(
                By.XPATH,
                '//*[@id="content"]/div/div[2]/div[1]/div/div[2]/div[2]/div/button[2]',
            )
            apply_button.click()
            ...
            self.handle_error(
                "Error while finishing registration", "finish_registration.png"
            )
    ...
    def register(self, register_retries=0):
        ...
        self.logger.info(
            f"Registering attempt # {register_retries + 1} {self.posh_user.username}"
        )
        try:
            ...
            gender_options = self.web_driver.find_elements(
                By.CLASS_NAME, "dropdown__link"
            )
            ...
            # Submit the form
            image_name = (
                f"/log_images/{self.campaign.title}/register_form_submitted.png"
            )
            self.web_driver.save_screenshot(image_name)
            ...
            response = self.requests_session.get(
                f"https://poshmark.com/closet/{self.posh_user.username}", timeout=30
            )
    ...
    def login(self, login_retries=0):
        ...
        try:
            ...
            self.logger.info(
                f"Login attempt # {login_retries + 1} for {self.posh_user.username}"
            )

            self.web_driver.get("https://poshmark.com/login")

            self.logger.info(
                f"At login page - Current URL: {self.web_driver.current_url}"
            )
    ...
    def go_to_closet(self, username=None):
        ...
        try:
            if (
                ...
                != f"https://poshmark.com/closet/{posh_username}"
            ):
            ...
            show_all_listings_xpath = (
                '//*[@id="content"]/div/div[2]/div/div/section/div[2]/div/div/button'
            )
    ...
    def get_all_listings(self):
        ...
        try:
            ...
            self.logger.info(
                f"Getting all listings for {self.campaign.posh_user.username}"
            )
            ...
            if self.is_present(By.CLASS_NAME, "card--small"):
                ...
                for listed_item in all_listed_items:
                    ...
                    try:
                        icon = listed_item.find_element(
                            By.CLASS_NAME, "inventory-tag__text"
                        )
                    except NoSuchElementException:
                        icon = None

                    listing = {"id": listing_id, "title": title.text}
            ...
            for type_key, listings in listed_items.items():
                ...
                for listing in listings:
                    grid += "{:<20} {:<20} {}\n".format(
                        type_str, listing["id"], listing["title"]
                    )
            ...
            self.handle_error(
                "Error while getting all listings", "get_all_listings_error.png"
            )
    ...
    def update_profile(self, update_profile_retries=0):
        ...
        try:
            self.logger.info(
                f"Profile update Attempt # {update_profile_retries + 1} for {self.campaign.posh_user.username}"
            )
            ...
            if self.posh_user.header_picture:
                ...
                self.bucket.download_file(
                    self.posh_user.header_picture.name, header_picture_name
                )

                header_picture = self.locate(
                    By.CLASS_NAME, "image-selector__input-img-files"
                )
                ...
                apply_button = self.locate(
                    By.XPATH,
                    '//*[@id="content"]/div/div[2]/div/div[1]/div[2]/div/div[2]/div[2]/div/button[2]',
                )
                apply_button.click()
            ...
            self.handle_error(
                "Error while updating profile", "update_profile_error.png"
            )
            return False
    ...
    def list_item(self, listed_item: ListedItem, listing_images, list_item_retries=0):
        ...
        try:
            ...
            self.logger.info(
                f"Attempt # {list_item_retries + 1} to list {listing_title} for {self.posh_user.username}"
            )
            ...
            if self.is_present(By.XPATH, '//*[@id="app"]/main/div[1]/div/div[2]'):
                ...
                self.web_driver.save_screenshot(image_path)
                self.logger.error(
                    "Error encountered when on the new listing page. Setting user inactive.",
                    image_path,
                )
                self.posh_user_inactive()
                ...
                self.bucket.download_file(
                    listing.cover_photo.name,
                    f"{listing_folder }/{listing_cover_photo_name}",
                )

                for listing_image in listing_images:
                    image_name = listing_image.image.name.split("/")[-1]
                    self.bucket.download_file(
                        listing_image.image.name, f"{listing_folder }/{image_name}"
                    )
                ...
                while wait_attempts < 5 and not self.is_present(
                ...
                category_dropdown = self.locate(By.XPATH, category_xpath)
                ...
                subcategory_menu = self.locate(
                    By.CLASS_NAME, "dropdown__menu--expanded"
                )
                ...
                size_dropdown = self.locate(
                    By.XPATH,
                    '//*[@id="content"]/div/div[1]/div[2]/section[4]/div[2]/div[2]/div[1]/div/div[2]/div[1]/div',
                )
                ...
                size_dropdown.click()
                size_buttons = self.locate_all(
                    By.CLASS_NAME, "navigation--horizontal__tab"
                )
                ...
                title_field = self.locate(
                    By.XPATH,
                    '//*[@id="content"]/div/div[1]/div[2]/section[2]/div[1]/div[2]/div/div[1]/div/div/input',
                )
                description_field = self.locate(
                    By.XPATH,
                    '//*[@id="content"]/div/div[1]/div[2]/section[2]/div[2]/div[2]/textarea',
                )
                ...
                for part in listing_description.split("\n"):
                    ...
                    ActionChains(self.web_driver).key_down(Keys.SHIFT).key_down(
                        Keys.ENTER
                ...
                if listing_tags:
                    tags_button = self.locate(
                        By.XPATH,
                        '//*[@id="content"]/div/div[1]/div[2]/section[5]/div/div[2]/div[1]/button[1]',
                        "clickable",
                    )
                    self.web_driver.execute_script("arguments[0].click();", tags_button)

                next_button = self.locate(
                    By.XPATH, '//*[@id="content"]/div/div[1]/div[2]/div[2]/button'
                )
                next_button.click()
                ...
                list_item_button = self.locate(
                    By.XPATH,
                    '//*[@id="content"]/div/div[1]/div[2]/div[3]/div[2]/div[2]/div[2]/button',
                )
                list_item_button.click()

                sell_button = self.is_present(
                    By.XPATH, '//*[@id="app"]/header/nav[2]/div[1]/ul[2]/li[2]/a'
                )
        ...
        try:
            ...
            if self.is_present(By.CLASS_NAME, "g-recaptcha-con"):
                ...
                if retries == 5 and captcha_response is None:
                    ...
                    share_button = self.locate(
                        By.CLASS_NAME, "social-action-bar__share"
                    )
                    share_button.click()
                    ...
                    to_followers_button = self.locate(
                        By.CLASS_NAME, "internal-share__link"
                    )
                    to_followers_button.click()
    ...
    def get_listed_item_id(self, listing_title):
        ...
        for listing in listings:
            ...
            if (
                title == listing_title
                and not ListedItem.objects.filter(
            ...
            if self.is_present(By.CLASS_NAME, "active-offers__content"):
                ...
                for offer in offers:
                    if listing_title in offer.text:
                        ...
                        listing_price_text = self.locate(
                            By.XPATH,
                            '//*[@id="content"]/div/div[2]/div[2]/div[1]/div/div[2]/h5[2]',
                        ).text
                        listing_price = int(re.findall(r"\d+", listing_price_text)[-1])

                        active_offers = self.locate_all(
                            By.CLASS_NAME, "active-offers__content"
                        )
                        offer_page_url = self.web_driver.current_url

                        self.logger.info(
                            f"There are currently {len(active_offers)} active offers"
                        )
                        for x in range(len(active_offers)):
                            ...
                            try:
                                ...
                                for chat_bubble in reversed(chat_bubbles):
                                    try:
                                        bubble = chat_bubble.find_element(
                                            By.XPATH, ".//*"
                                        )
                                        if sender_offer and receiver_offer:
                                            break
                                        elif (
                                            "sender" in bubble.get_attribute("class")
                                            and not sender_offer
                                        ):
                                            text = bubble.text
                                            if "offered" in text:
                                                sender_offer = int(
                                                ...
                                                self.logger.warning(
                                                    f'Seller cancelled. Message: "{text}"'
                                                )
                                                break
                                            else:
                                                self.logger.warning(
                                                    f'Unknown message sent by seller. Message: "{text}"'
                                                )
                                                break
                                        elif (
                                            "receiver" in bubble.get_attribute("class")
                                            and not receiver_offer
                                        ):
                                            text = bubble.text
                                            if "declined" in text:
                                ...
                                if sender_offer:
                                    if (
                                        sender_offer >= lowest_price
                                        or sender_offer >= receiver_offer - 1
                                    ):
                                        primary_buttons = self.locate_all(
                                        ...
                                        for button in primary_buttons:
                                            if button.text == "Yes":
                                                button.click()
                                                self.logger.info(
                                                    f"Accepted offer at ${sender_offer}."
                                                )
                                                self.sleep(5)

                                                return True
                                    else:
                                        secondary_buttons = self.locate_all(
                                        ...
                                        if receiver_offer < lowest_price - 4:
                                            ...
                                            counter_offer_input = self.locate(
                                                By.CLASS_NAME, "form__text--input"
                                            )
                                            counter_offer_input.send_keys(
                                                str(counter_offer)
                                            )
                                            self.sleep(2)
                                            primary_buttons = self.locate_all(
                                            ...
                                            for button in primary_buttons:
                                                if button.text == "Submit":
                                                    button.click()
                                                    self.logger.info(
                                                        f"Buyer offered ${sender_offer}, countered offer sent for ${counter_offer}"
                                                    )
                                                    self.sleep(5)
                                                    break
                                else:
                                    self.logger.warning(
                                        "Nothing to do on the current offer"
                                    )
                                    self.logger.debug(
                                        f"Our Offer: ${receiver_offer} Sender Offer: ${sender_offer}"
                                    )
                            except TimeoutException:
                                self.logger.warning(
    ...
    def send_offer_to_likers(self, listed_item):
        ...
        try:
            self.logger.info(
                f"Sending offers to all likers for the following item: {listed_item.listing_title}"
            )
            ...
            if not first_offer:
                ...
                offer_to_likers_button = self.locate(
                    By.XPATH, "//button[@data-et-name='make_offer_to_likers']"
                )
                ...
                if self.is_present(
                    By.XPATH,
                    "//h4[contains(text(), 'Sorry, this listing has no Likers. Share it to get more likes!')]",
                ):
                    self.logger.info(
                        "Pop up came up: Sorry, this listing has no Likers. Share it to get more likes!"
                    )
                ...
                listing_price = int(
                    offer_modal.find_element(By.CSS_SELECTOR, ".fw--med").text.strip(
                        "$"
                    )
                )

                offer_amount = int(listing_price - (listing_price * 0.1))
                ...
                shipping_dropdown = self.locate(
                    By.XPATH,
                    '//*[@id="content"]/div/div/div[3]/div[2]/div[5]/div[2]/div/div[2]/div[1]/div[2]/div[2]/div/form/div[2]/div[1]/div/div/div/div[1]/div',
                )
                shipping_dropdown.click()

                shipping_options = self.locate_all(
                    By.CLASS_NAME, "dropdown__menu__item"
                )
                ...
                apply_button = self.locate(
                    By.XPATH,
                    '//*[@id="content"]/div/div/div[3]/div[2]/div[5]/div[2]/div/div[2]/div[1]/div[2]/div[3]/div/button[2]',
                )
                apply_button.click()

                done_button = self.locate(
                    By.XPATH,
                    '//*[@id="content"]/div/div/div[3]/div[2]/div[5]/div[2]/div/div[2]/div[2]/div[3]/button',
                )
                done_button.click()
            ...
            self.handle_error(
                "Error while sending offer to likers", "send_offer_to_likers.png"
            )
    ...
    def check_comments(self, listed_item_id, listed_item_title):
        ...
        try:
            self.logger.info(
                f"Checking the comments for the following item: {listed_item_title}"
            )
            ...
            for closet in closets_to_report:
                ...
                self.logger.info(
                    f'Reporting {closet[0].split("/")[-1]} for {report_type}'
                )
                ...
                ellipses_dropdown = self.web_driver.find_element(
                    By.CLASS_NAME, "ellipses-dot"
                )
                ellipses_dropdown.click()

                # Find the "Block User" option in the dropdown and click it
                dropdown_menu = self.web_driver.find_element(
                    By.CLASS_NAME, "dropdown__menu--expanded"
                )
                block_user_option = dropdown_menu.find_element(
                    By.XPATH, "//div[contains(text(), 'Report User')]"
                )
                block_user_option.click()

                # Find the select element within the modal
                select_element = self.web_driver.find_element(
                    By.CLASS_NAME, "flag-modal__select"
                )
    ...
    def follow_random_follower(self):
        ...
        try:
            ...
            followers_button = self.locate(
                By.XPATH,
                '//*[@id="content"]/div/div/div[1]/div/div/div[2]/div/div/nav/ul/li[3]/div',
            )
            ...
            while not selected_user:
                ...
                try:
                    if follow_button.text.replace(" ", "") == "Follow":
                        selected_user = selection.find_element(
                            By.CLASS_NAME, "follow__action__follower "
                        ).text
                        actions = ActionChains(self.web_driver)
                        actions.move_to_element(follow_button).perform()

                        self.logger.info(
                            f"The following user was selected to be followed: {selected_user}"
                        )
    ...
    def follow_random_user(self):
        ...
        try:
            ...
            selected_users = (
                random.sample(available_users, sample_size) if available_users else []
            )

            for selected_user in selected_users:
                try:
                    username = selected_user.find_element(
                        By.CLASS_NAME, "follow__action__follower"
                    ).text
                    follow_button = selected_user.find_element(
                        By.CLASS_NAME, "follow__action__button"
                    )

                    self.logger.info(
                        f"The following user was selected to be followed: {username}"
                    )

                    self.web_driver.execute_script(
                        "return arguments[0].scrollIntoView(true);", follow_button
                    )
    ...
    def report_listing(self, listing_id, report_type):
        ...
        try:
            ...
            dropdown = modal.find_element(
                By.CSS_SELECTOR, "div.flag-modal__select [data-test='dropdown']"
            )
            dropdown.click()

            # Locate the option you want to select and click on it using text contains
            option_to_select = modal.find_element(
                By.XPATH,
                f"//li[contains(@class, 'dropdown__menu__item')]/div[contains(text(), '{report_type}')]",
            )
            option_to_select.click()

            if report_type == ListedItemToReport.MISTAGGED_ITEM:
                # Click on the second dropdown to open its options
                second_dropdown = modal.find_element(
                    By.XPATH, "//div[@data-test='select'][2]/div[@data-test='dropdown']"
                )
                second_dropdown.click()

                # Locate and click on "Mistagged Condition"
                mistagged_condition_option = modal.find_element(
                    By.XPATH,
                    "//li[@class='dropdown__menu__item']/div[contains(text(), 'Mistagged Condition')]",
                )
            ...
            self.handle_error(
                "Error while reporting listing", "report_listing_error.png"
            )
    ...
    def comment_on_listing(self, listing_id, comment):
        ...
        try:
            self.logger.info(
                f"Adding comment to the following item: Listing ID - {listing_id} Comment - {comment}"
            )
            ...
            submit_button = self.locate(
                By.XPATH, '//*[@id="commentForm"]/div[2]/button'
            )
            submit_button.click()
            ...
            self.handle_error(
                "Error while commenting on listing", "comment_on_listing_error.png"
            )
    ...
    def send_private_bundle_message(self, listing_id, message):
        ...
        try:
            self.logger.info(
                f"Adding the following item to a bundle and sending a message: Listing Id - {listing_id} Message - {message}"
            )
            ...
            bundle_icon = self.locate(
                By.CLASS_NAME, "bundle-medium-gray-with-plus-sign"
            )
            bundle_icon.click()
            ...
            self.handle_error(
                "Error while adding listing to bundle and sending message",
                "send_private_bundle_message_error.png",
            )
            ...
            for x in range(random.randint(15, 35)):
                ...
                for post in posts:
                    if post.is_displayed():
                        try:
                            like_icon = post.find_element(
                                By.CLASS_NAME, "heart-gray-empty"
                            )
                            share_icon = post.find_element(
                                By.CLASS_NAME, "share-gray-large"
                            )
                            post_title = post.find_element(
                                By.CLASS_NAME, "feed__unit__header__title--medium"
                            ).text
                            listing_title = post.find_element(
                                By.CLASS_NAME, "feed__summary__title-block"
                            ).text
                            ...
                            if random.random() < 0.30:
                                ...
                                self.logger.info(
                                    f"Just liked {listing_title} posted by {username}"
                                )
                            ...
                            if random.random() < 0.30:
                                ...
                                to_my_followers = self.locate(
                                    By.CLASS_NAME, "share-wrapper__icon-container"
                                )
                                to_my_followers.click()

                                self.logger.info(
                                    f"Just shared {listing_title} posted by {username}"
                                )
    ...
    def find_bad_listings(self):
        ...
        for brand in random.sample(brands, 1):
            for category in categories:
                items_reviewed = 0
                self.logger.info(
                    f"Searching for bad listings in the following brand: {brand}"
                )

                self.web_driver.get(
                    f"https://poshmark.com/brand/{brand}-{category}?price%5B%5D={min_price}-{max_price}"
                )
                ...
                while number_of_pages < 10 and items_reviewed <= 200:
                    ...
                    listed_items_container = self.web_driver.find_element(
                        By.CLASS_NAME, "tiles_container"
                    )
                    listed_items = listed_items_container.find_elements(
                        By.CLASS_NAME, "col-x12"
                    )

                    for listed_item in listed_items:
                        items_reviewed += 1
                        try:
                            closet = listed_item.find_element(
                                By.CSS_SELECTOR, "a.tile__creator"
                            )
                            closet_url = closet.get_attribute("href")
                            username = closet_url.split("/")[-1]
                            if (
                                closet_url not in posh_ambassadors
                                and username not in usernames
                            ):
                                listing_id = listed_item.get_attribute(
                                    "data-et-prop-listing_id"
                                )
                                listing_link = listed_item.find_element(
                                    By.TAG_NAME, "a"
                                )
                                listing_url = listing_link.get_attribute("href")
                                listing_title = listed_item.find_element(
                                    By.CSS_SELECTOR, "a.tile__title"
                                ).text.strip()
                                closet_response = requests.get(closet_url)
                                if closet_response.status_code == 200:
                                    closet_soup = BeautifulSoup(
                                        closet_response.content, "html.parser"
                                    )
                                    badge_div = closet_soup.find(
                                        "div", {"badgepresentation": "[object Object]"}
                                    )
                    ...
                    try:
                        next_button = self.web_driver.find_element(
