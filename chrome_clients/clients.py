...
class BaseClient:
    ...
    def open(self):
        ...
        self.web_driver = webdriver.Chrome(service=service, options=self.web_driver_options)
        ...
        try:
            ...
            if self.is_present(By.CLASS_NAME, 'g-recaptcha-con'):
                ...
                if retries == 5 and captcha_response is None:
                    ...
                    self.logger.info(f'2Captcha successfully solved captcha after {retries} {word}')
                    # Set the captcha response
                    self.web_driver.execute_script(f"document.querySelector('#g-recaptcha-response').value = token")
