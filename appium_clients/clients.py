...
class AppiumClient:
    ...
    def open(self):
        ...
        self.driver = webdriver.Remote(
            APPIUM_SERVER_URL, options=self.capabilities_options
        )
    ...
    def get_current_app_package(self):
        ...
        current_focus = windows[
            current_focus_index : end_of_current_focus + current_focus_index
        ]
...
class PoshMarkClient(AppiumClient):
    ...
    def download_and_send_file(self, key, download_folder):
        ...
        self.send_file(
            ...
            source_path=download_location,
        )

    def alert_check(self):
        self.logger.info("Checking for an alert")

        if not self.posh_party_alert_dismissed and self.is_present(
            ...
            if "party" in title.lower():
                ...
                self.logger.info(
                    f"No handler for alert with the following title: {title}"
                )
            ...
            if "party" in message.text.lower():
                ...
                self.logger.info(
                    f"Alert with the following message popped up: {message.text}"
                )
            ...
            close_button = self.locate(
                AppiumBy.ID, "com.poshmark.app:id/tooltip_close_button"
            )
            close_button.click()
    ...
    def tap_img(self, name):
        search_box = self.locate(
            AppiumBy.ID, "com.google.android.documentsui:id/searchbar_title"
        )
        self.click(search_box)

        search_input = self.locate(
            AppiumBy.ID, "com.google.android.documentsui:id/search_src_text"
        )
        ...
        try:
            ...
            while not self.is_registered and retries < 4:
                ...
                if not self.is_present(AppiumBy.ID, "titleTextView"):
                    self.logger.info(
                        "No screen title elements, probably at init screen."
                    )
                    ...
                    if not self.account_pop_up_clicked and self.is_present(
                        AppiumBy.ID, "com.google.android.gms:id/cancel"
                    ):
                        none_of_the_above = self.locate(
                            AppiumBy.ID, "com.google.android.gms:id/cancel"
                        )
                        self.click(none_of_the_above)
                        self.logger.info(
                            "Google accounts pop up. Clicked None of the above."
                        )
                    ...
                    if window_title and window_title.text == "Get Started":
                        ...
                        if not self.profile_picture_added:
                            ...
                            profile_picture_key = (
                                self.campaign.posh_user.profile_picture.name
                            )
                            ...
                            attempts = 0
                            while (
                                self.is_present(AppiumBy.ID, "progressBar")
                                and attempts < 4
                            ):
                        ...
                        self.input_text(
                            username, self.campaign.posh_user.username, False
                        )
                        ...
                        if self.is_present(AppiumBy.ID, "popupContainer"):
                            ...
                            self.logger.info(
                                f"Looks like username was taken. Choosing new username: {new_username.text}"
                            )
                    ...
                    elif window_title and window_title.text in (
                        "Sizes",
                    ...
                    if self.is_present(AppiumBy.ID, "textinput_error"):
                        ...
                        self.logger.error(
                            f"The following form error was found (Campaign will be stopped): {error.text}"
                        )

                        if (
                            "email address is already tied to another user"
                            in error.text
                            or "Please enter a valid email address" in error.text
                        ):
                            self.logger.info(
                                "Setting the user inactive since the email is taken or it is invalid."
                            )
                    ...
                    ) and not self.is_present(AppiumBy.ID, "titleTextView"):
    ...
    def finish_registration(self):
        try:
            ...
            while not self.is_present(AppiumBy.ID, "sellTab"):
                ...
                if window_title and window_title.text in (
                    "Sizes",
                    "Complete your Profile",
                ):
                    ...
                    try:
                        if self.is_present(AppiumBy.ID, "brandLogo"):
                            ...
                            brands = self.locate_all(
                                AppiumBy.ID, "suggestedBrandLogo3"
                            )[:9]
    ...
    def list_item(self, item_to_list: ListedItem, listing_images: List[ListingImage]):
        try:
            ...
            self.logger.info(
                f"Listing {listing.title} for {self.campaign.posh_user.username}"
            )
            self.listed = False
            new_listing = False
                ...
                if self.is_present(AppiumBy.ID, "sellTab"):
                    ...
                    if window_title and window_title.text == "Listing Details":
                        if not added_media_items and len(listing_images) > 0:
                            ...
                            if not new_listing:
                                added_images_found = self.scroll_until_found(
                                    AppiumBy.XPATH,
                                    f"//*[contains(@text, 'PHOTOS & VIDEO')]",
                                    direction="up",
                                )
                                if added_images_found:
                                    added_images = self.locate_all(
                                        AppiumBy.ID, "container"
                                    )
                            ...
                            if added_images_found:
                                if listing_images:
                                    for listing_image in listing_images:
                                        ...
                                        self.download_and_send_file(
                                            image_key, listing_folder
                                        )

                                    self.sleep(5)

                                    add_more_button = self.locate(
                                        AppiumBy.ID, "add_more"
                                    )
                                    ...
                                    if self.is_present(
                                        ...
                                        self.logger.info(
                                            "Denied access to camera permanently"
                                        )
                                    ...
                                    while counter < len(listing_images):
                                        try:
                                            img = self.locate(
                                                AppiumBy.XPATH,
                                                "//android.widget.ImageView[@resource-id='com.google.android.documentsui:id/icon_thumb' and @selected='false']",
                                            )
                                            if counter == 0:
                                                self.long_click(img)
                                            else:
                                            ...
                                            self.logger.warning(
                                                f"Could not find another image"
                                            )

                                        if (
                                            counter != 0
                                            and counter % 5 == 0
                                            and counter != len(listing_images) - 1
                                        ):
                                            self.swipe("up", 580 * 3)
                                    ...
                                    self.logger.info(
                                        "All listing images seem to be in there already. Skipping..."
                                    )

                                added_media_items = True
                            else:
                                self.logger.info(
                                    "Listing images were not sent because could not check if there were any previously uploaded"
                                )
                        ...
                        if not added_description:
                            ...
                            description_body = self.locate(
                                AppiumBy.ID, "description_body"
                            )

                            if "required" in description_body.text.lower():
                                self.click(description_body)

                                description_input = self.locate(
                                    AppiumBy.ID, "description_editor"
                                )
                                description_input.clear()
                                self.send_keys(
                                    description_input, str(listing.description)
                                )
                                ...
                                self.logger.info(
                                    "Description has already been added. Skipping..."
                                )
                        ...
                        if not added_category:
                            ...
                            added_category_found = self.scroll_until_found(
                                AppiumBy.ID, "catalog_edit_text"
                            )
                            if added_category_found:
                                ...
                                if "required" in category.text.lower():
                                    ...
                                    primary_category = (
                                        listing_category[:space_index]
                                        .replace(" ", "_")
                                        .lower()
                                    )
                                    secondary_category = (
                                        listing_category[space_index + 1 :]
                                        .replace(" ", "_")
                                        .lower()
                                    )
                                    subcategory = listing.subcategory.replace(
                                        " ", "_"
                                    ).lower()
                                    ...
                                    primary_category_button = self.locate(
                                    ...
                                    while (
                                        not secondary_category_clicked
                                        and secondary_category_click_attempts < 7
                                    ):
                                        if self.is_present(
                                            AppiumBy.ACCESSIBILITY_ID,
                                            secondary_category,
                                            ...
                                            secondary_category_button = self.locate(
                                                AppiumBy.ACCESSIBILITY_ID,
                                                secondary_category,
                                            )
                                    ...
                                    if (
                                        secondary_category_click_attempts >= 7
                                        and not secondary_category_clicked
                                    ):
                                        return False
                                    ...
                                    while (
                                        not subcategory_clicked
                                        and subcategory_click_attempts < 7
                                    ):
                                        subcategory_click_attempts += 1
                                        if self.is_present(
                                            AppiumBy.ACCESSIBILITY_ID, subcategory
                                        ):
                                            subcategory_button = self.locate(
                                                AppiumBy.ACCESSIBILITY_ID, subcategory
                                            )
                        ...
                        while (
                            not self.is_present(AppiumBy.ID, "titleTextView")
                            or self.locate(AppiumBy.ID, "titleTextView").text
                            != "Listing Details"
                        ):
                            for _ in range(3):
                                self.driver.back()
                        ...
                        if not added_size:
                            ...
                            size_input_found = self.scroll_until_found(
                                AppiumBy.ID, "size_edit_text"
                            )
                            ...
                            if size_input_found:
                                ...
                                if listing.size.lower() == "os":
                                    one_size = self.locate(
                                        AppiumBy.ACCESSIBILITY_ID, "One Size"
                                    )
                                    self.click(one_size)
                                    added_size = True
                                    ...
                                    if (
                                        "belt"
                                        not in listing.category.lower()
                                        + listing.subcategory.lower()
                                    ):
                                        package_name = self.capabilities["appPackage"]
                                        selected_size_category = self.locate(
                                            AppiumBy.XPATH,
                                            f"//android.widget.TextView[@resource-id='{package_name}:id/size_tab_title_text' and @selected='true']",
                                        )
                                        while (
                                            self.is_present(
                                                AppiumBy.XPATH,
                                            ...
                                            and selected_size_category.text != "CUSTOM"
                                        ):
                                            self.logger.info(
                                                f"Looking for size in {selected_size_category.text} category"
                                            )
                                            size_found = self.scroll_until_found(
                                                AppiumBy.ACCESSIBILITY_ID,
                                                listing.size,
                                                max_scrolls=4,
                                            )
                                    ...
                                    if not added_size:
                                        custom_size_button = self.locate(
                                            AppiumBy.ACCESSIBILITY_ID, "Custom"
                                        )
                                        self.click(custom_size_button)

                                        add_option = self.locate(
                                            AppiumBy.ID, "container"
                                        )
                                        self.click(add_option)

                                        custom_size_input = self.locate(
                                            AppiumBy.ID, "messageText"
                                        )
                                        ...
                                        self.logger.info(
                                            f"Put in {listing.size} for the size"
                                        )
                        ...
                        if not added_brand and listing.brand:
                            ...
                            brand_edit_found = self.scroll_until_found(
                                AppiumBy.ID, "brand_edit_text"
                            )

                            if brand_edit_found:
                                brand_input = self.locate(
                                    AppiumBy.ID, "brand_edit_text"
                                )
                                while (
                                    brand_edit_found
                                    and brand_input.text.lower()
                                    != listing.brand.lower()
                                ):
                                    self.click(brand_input)
                                    self.logger.info("Clicked brand button")

                                    brand_search = self.locate(
                                        AppiumBy.ID, "searchTextView"
                                    )
                                    ...
                                    if not self.is_present(AppiumBy.XPATH, brand_xpath):
                                        self.logger.info(
                                            "Brand did not pop up on search... Taping back."
                                        )
                                    else:
                        ...
                        if not added_original_price:
                            original_price_found = self.scroll_until_found(
                                AppiumBy.ID, "original_price_edit_text"
                            )

                            if original_price_found:
                                original_price = self.locate(
                                    AppiumBy.ID, "original_price_edit_text"
                                )
                                self.input_text(
                        ...
                        if not added_listing_price:
                            listing_price_found = self.scroll_until_found(
                                AppiumBy.ID, "listing_price_edit_text"
                            )

                            if listing_price_found:
                                listing_price = self.locate(
                                    AppiumBy.ID, "listing_price_edit_text"
                                )
                                self.input_text(
                            ...
                            earnings_str = self.locate(
                                AppiumBy.ID, "earnings_edit_text"
                            ).text
                        ...
                        while not sell_button_present and list_attempts < 20:
                            sell_button_present = self.is_present(
                                AppiumBy.ID, "sellTab"
                            )
                            list_attempts += 1

                            if self.is_present(AppiumBy.ID, "android:id/message"):
                                error_message = self.locate(
                                    AppiumBy.ID, "android:id/message"
                                )
                                self.logger.warning(
                                    f"A pop up came up with the following message: {error_message.text}"
                                )

                                if (
                                    "you cannot currently perform this request"
                                    in error_message.text.lower()
                                ):
                                    self.logger.info(
                                        "User is inactive and cannot list items. Setting inactive..."
                                    )
                                    self.campaign.posh_user.is_active_in_posh = False
                                    self.campaign.posh_user.save(
                                        update_fields=["is_active_in_posh"]
                                ...
                                AppiumBy.XPATH,
                                f"//*[contains(@text, 'Certify Listing')]",
                            ):
                                self.logger.warning(
                                    "Certify listing page came up. Clicking certify."
                                )
                                certify_button = self.locate(
                                    AppiumBy.XPATH,
                                    f"//*[contains(@text, 'Certify Listing')]",
                                )
                                ...
                                AppiumBy.ID, "com.poshmark.app:id/close"
                            ):
                                close_button = self.locate(
                                    AppiumBy.ID, "com.poshmark.app:id/close"
                                )
                        ...
                        window_title
                        or self.is_present(AppiumBy.ID, "actionbar_closet_layout")
                        or self.is_present(AppiumBy.ID, "actionbarTitleLayout")
                        or self.is_present(AppiumBy.ID, "select_a_brand_title")
                    ):
                        for _ in range(3):
                            self.driver.back()
                            self.sleep(0.2)
                        ...
                        AppiumBy.XPATH,
                        "//*[starts-with(@resource-id, 'com.google.android.documentsui:id')]",
                    ):
    ...
    def get_listed_item_id(self):
        ...
        try:
            ...
            first_share_button = self.locate(
                AppiumBy.XPATH,
                f"//*[@resource-id='{self.capabilities['appPackage']}:id/shareButton']",
            )
...
class ProxyDroidClient(AppiumClient):
    def __init__(self, device: Device, logger, proxy: Proxy):
        ...
        super(ProxyDroidClient, self).__init__(
            device.serial,
            device.system_port,
            device.mjpeg_server_port,
            logger,
        ...
        while any(elem not in values_checked for elem in values_to_check.keys()):
            ...
            for container in containers:
                try:
                    ...
                    summary_element = container.find_element(
                        AppiumBy.ID, "android:id/summary"
                    )
                    current_title = title_element.text

                    if (
                        current_title in values_to_check.keys()
                        and current_title not in values_checked
                        and summary_element.text != values_to_check[current_title]
                    ):
                        title_element.click()
...
class IDChangerClient(AppiumClient):
    def __init__(self, device: Device, logger):
        ...
        super(IDChangerClient, self).__init__(
            device.serial,
            device.system_port,
            device.mjpeg_server_port,
            logger,
...
class SwiftBackupClient(AppiumClient):
    def __init__(self, device: Device, logger, posh_user: PoshUser):
        ...
        super(SwiftBackupClient, self).__init__(
            device.serial,
            device.system_port,
            device.mjpeg_server_port,
            logger,
    ...
    def reset_data(self):
        ...
        search_icon = self.locate(
            AppiumBy.ID, "org.swiftapps.swiftbackup:id/action_search"
        )
        search_icon.click()

        search_bar = self.locate(
            AppiumBy.ID, "org.swiftapps.swiftbackup:id/searchEditText"
        )
    ...
    def _download_backup_files(self):
        ...
        bucket.download_file(
            app_data.backup_data.name, f"{self.location}/com.poshmark.app.dat"
        )
        bucket.download_file(
            app_data.xml_data.name, f"{self.location}/com.poshmark.app.xml"
        )
    ...
    def save_backup(self):
        ...
        while (
            not self.is_present(
                AppiumBy.ID, "org.swiftapps.swiftbackup:id/tv_progress_message"
            )
            or self.locate(
                AppiumBy.ID, "org.swiftapps.swiftbackup:id/tv_progress_message"
            ).text
        ...
        done_button = self.locate(
            AppiumBy.ID, "org.swiftapps.swiftbackup:id/btn_action"
        )
        done_button.click()
        ...
        xml_file_location = f"{self.location}/com.poshmark.app.xml"
        ...
        with open(data_file_location, "rb") as data_file, open(
            xml_file_location, "rb"
        ) as xml_file:
            if AppData.objects.filter(posh_user=self.posh_user).exists():
                app_data = AppData.objects.filter(posh_user=self.posh_user).first()
            else:
                app_data = AppData(posh_user=self.posh_user, type=AppData.POSHMARK)

            app_data.backup_data.save(
                "com.poshmark.app.dat", ContentFile(data_file.read()), save=False
            )
            app_data.xml_data.save(
                "com.poshmark.app.xml", ContentFile(xml_file.read()), save=True
            )

        # Delete backup
        more_info = self.locate(
            AppiumBy.XPATH,
            "/hierarchy/android.widget.FrameLayout/android.widget.LinearLayout/android.widget.FrameLayout/android.widget.LinearLayout/android.widget.FrameLayout/android.widget.ScrollView/android.widget.LinearLayout/androidx.cardview.widget.CardView[2]/android.widget.LinearLayout/android.view.ViewGroup/android.view.ViewGroup/android.widget.ImageView",
        )
        more_info.click()

        delete_backup = self.locate(
            AppiumBy.XPATH,
    ...
    def load_backup(self):
        ...
        device.push(
            f"{self.location}/com.poshmark.app.dat",
            "/storage/emulated/0/SwiftBackup/accounts/8690a48a4fcc72f1/backups/apps/local/com.poshmark.app.dat",
        )
        device.push(
            f"{self.location}/com.poshmark.app.xml",
            "/storage/emulated/0/SwiftBackup/accounts/8690a48a4fcc72f1/backups/apps/local/com.poshmark.app.xml",
        )
        ...
        # Restore backup
        restore_button = self.locate(
            AppiumBy.ID, "org.swiftapps.swiftbackup:id/btn_restore"
        )
